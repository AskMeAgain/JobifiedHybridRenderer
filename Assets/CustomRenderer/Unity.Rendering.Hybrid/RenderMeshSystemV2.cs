...
namespace CustomRenderer.Unity.Rendering.Hybrid
{
    ...
    public class RenderMeshSystemV2 : JobComponentSystem
    {
        ...
        private void OldMethod(FrozenRenderSceneTag tag, NativeArray<ArchetypeChunk> chunks, int sharedRenderCount,
                               NativeArray<int> sharedRendererCounts, NativeArray<int> sortedChunkIndices,
            ...
            for (int i = 0; i < sharedRenderCount; i++)
            {
                ...
                while (sortedChunkIndex < endSortedChunkIndex)
                {
                    ...
                    while (remainingEntitySlots > 0)
                    {
                        ...
                        #if UNITY_EDITOR
                        if (editorRenderDataIndex != nextChunk.GetSharedComponentIndex(editorRenderDataType)) break;
                        #endif
        ...
        void UpdateFrozenRenderBatches()
        {
            ...
            if (staticChunksOrderVersion == m_LastFrozenChunksOrderVersion) return;
            ...
            for (var i = 0; i < loadedSceneTags.Count; i++)
            {
                ...
                if (alreadyTrackingSubscene) continue;
